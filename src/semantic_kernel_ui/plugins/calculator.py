...
def _safe_eval(node, variables):
    ...
    if isinstance(node, ast.UnaryOp):
        ...
        if op_type not in _ALLOWED_OPERATORS:
            raise ValueError("Operator not allowed")
        return _ALLOWED_OPERATORS[op_type](_safe_eval(node.operand, variables))
...
class CalculatorPlugin:
    ...
    @kernel_function(name="calculate", description="Perform mathematical calculations")
    def calculate(self, expression: Annotated[str, "Mathematical expression to evaluate"]) -> Annotated[str, "Result of the calculation"]:
        try:
            ...
            last_value = None
            for part in parts:
                if '=' in part and part.count('=') == 1 and not part.strip().startswith('=='):
    ...
    @kernel_function(name="convert_units", description="Convert between different units")
    def convert_units(
        self,
